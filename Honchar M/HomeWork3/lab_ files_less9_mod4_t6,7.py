...
# 2. Если имени нет в черном и белом списках, тогда добавить пользователя в whitelist.txt и вывести приветствие;
# 3. Если имя есть в белом списке, вывести приветствие;
# 4. Если имя в черном списке, тогда вывести сообщение, что имя в блоке.
# 5. Результат не должен зависеть от регистра введеных слов (строковые литералы).
#
f_name = str(input("Input your surname and name, please! ").title())
with open('whitelist.txt', 'r+', encoding= 'UTF-8') as file1, open('blacklist.txt', 'r+', encoding= 'UTF-8') as file2:
      ...
      if -1 < bl_list:
            ...
            file1.write(f_name)
            print(f"Hello, {f_name}! Your name added to whitelist our database! Access allowed!")
